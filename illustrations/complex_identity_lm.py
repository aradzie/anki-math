...
def main() -> None:
    ...
    masked_magnitude = np.ma.masked_where(
        magnitude < ORIGIN_MASK_RADIUS,
        magnitude,
    )
    ...
    facecolors[magnitude < ORIGIN_MASK_RADIUS, 3] = 0.0

    fig = plt.figure(figsize=(10, 8), constrained_layout=True)
    ax = fig.add_subplot(111, projection="3d")
    ax.set_title(r"$f(z)=z$: height $=\log |z|$, color $=\arg z$")
    ...
    ax.set_zlabel(r"$\log |z|$")
    ax.set_xlim(DOMAIN_MIN, DOMAIN_MAX)
    ax.set_ylim(DOMAIN_MIN, DOMAIN_MAX)
    ax.set_zlim(np.log(ORIGIN_MASK_RADIUS), np.log(np.sqrt(2 * DOMAIN_MAX**2)))
