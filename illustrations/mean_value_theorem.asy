...
real f(real x) { return 0.3*x*x; }

real a = 1;
real b = 5;
real c = (a + b) / 2;
real m = (f(b) - f(a)) / (b - a);
real secLabelX = a + 0.7 * (b - a);
pair secLabelPt = (secLabelX, f(a) + m * (secLabelX - a));
pair tanEnd = (c + 1.2, f(c) + m * 1.2);

// axes
draw((-0.5, 0)--(6, 0), Arrow(TeXHead));
draw((0, -0.5)--(0, 8.5), Arrow(TeXHead));
label("$x$", (6, 0), E);
label("$y$", (0, 8.5), N);
...
label("slope $=\frac{f(b)-f(a)}{b-a}$", secLabelPt, NW, heavygreen);
